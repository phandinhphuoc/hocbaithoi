...
def load_data(url):
    ...
    # Thử tải tối đa 3 lần nếu bị lỗi mạng (404 tạm thời)
    for _ in range(3):
        try:
...
st.markdown("""
    ...
    [data-testid="stVerticalBlockBorderWrapper"] { background-color: #161B22 !important; border: 1px solid #30363D !important; border-radius: 12px !important; }
...
if st.session_state.page == "Home":
    ...
    if df_topics is not None:
        # TỰ DÒ CỘT ID: Tìm bất cứ cột nào có chữ 'id'
        ...
        prefixes = ["TN_", "DS_", "SN_"]
        
        for i, tab in enumerate(tabs):
            with tab:
                filtered = df_topics[df_topics[id_col].str.upper().str.startswith(prefixes[i])]
        ...
        st.error("❌ App không thể kết nối với Google Sheets.")
        st.info("💡 Thầy kiểm tra: 1. Link Secrets có đúng không? 2. Đã chọn 'Bất kỳ ai có liên kết' chưa?")
...
# --- 5. TRANG LÀM BÀI ---
    ...
    if st.session_state.current_exam is None:
        ...
        if df_q is not None and df_c is not None:
            # Tự dò cột ID trong bảng Questions
